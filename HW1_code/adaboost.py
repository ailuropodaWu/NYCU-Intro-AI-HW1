...
class Adaboost:
    ...
    def selectBest(self, featureVals, iis, labels, features, weights):
        ...
        """
        Calculate the error of all features by the formula. And find the 
        smallest one, then return its error and the classifier.
        
        Variable:
          bestError:
            Used to record the smallest error currently, intial it to a big num
          error:
            Used to calculate the total error of a certain feature when it works on all dataset
          bestFeature:
            When finding a smaller bestError, update to that certain feature
          bestClf:
            The WeakClassifier made by the bestFeature
        """
        bestError = 1000
        for j in range(len(featureVals)) :
          ...
          for i in range(len(featureVals[j])) :
            if featureVals[j][i] < 0 :
              error += weights[i] * abs(1 - labels[i])
            else :
              error += weights[i] * abs(0 - labels[i])
          if error < bestError :
            bestError = error
            bestFeature = features[j]
        bestClf = WeakClassifier(bestFeature)
